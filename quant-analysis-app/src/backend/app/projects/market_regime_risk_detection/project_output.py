...
from app.pipeline.market_regime_and_risk_detection.metrics_pipeline_integration import \
    market_regime_metrics_calculation


# “Lambda’s work is: don’t execute now; execute only when I am explicitly called.”

async def market_regime_risk_detection_module_output(
        config: MarketPriceConfig,
) -> pd.DataFrame:
    cache_key = module_output_data_cache_key(
        f"{config.symbols}_market_regime_risk_detection",
        config.start,
        config.end,
        config.period,
        config.interval,
    ...
    async def compute():
        return await fetch_market_regime_data(config)

    market_regime = await get_module_output_cached_dataframe(
        cache_key=cache_key,
        compute_fn=compute,
    ...
    return require_dataframe(
        market_regime,
        context="market_regime_risk_detection_module_output",
    )


async def market_regime_metrics_module_output(
        config: MarketPriceConfig,
):
    cache_key = module_output_data_cache_key(
        f"{config.symbols}_market_regime_metrics",
        config.start,
        config.end,
        config.period,
        config.interval,
    ...
    async def compute():
        return await market_regime_metrics_calculation(config)

    regime_metrics = await module_output_cache_service(
        cache_key=cache_key,
        compute_fn=compute,
        ttl_seconds=settings.REDIS_TTL_MARKET_RETURNS_SECONDS,
    )
